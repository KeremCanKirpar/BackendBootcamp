...
public class EmailManager : IEmailService
{
    ...
    public async Task<ResponseDto<NoContent>> SendEmailAsync(string emailTo, string subject, string htmlBody)
    {
        try
        {
            ...
            {
                return ResponseDto<NoContent>.Fail("SMTP Sunucu adresi yapılanırılmamış", StatusCodes.Status500InternalServerError);
            }
            ...
            {
                return ResponseDto<NoContent>.Fail("SMTP şifresi  yapılanırılmamış", StatusCodes.Status500InternalServerError);
            }
