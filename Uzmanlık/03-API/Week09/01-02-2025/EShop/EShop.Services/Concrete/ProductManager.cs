...
public class ProductManager : IProductService
{
    ...
    public async Task<ResponseDto<NoContent>> HardDeleteAsync(int id)
    {
        try
        {
         ...
         var result =  await _unitOfWork.SaveAsync();
            if(result<1)
            {
                return ResponseDto<NoContent>.Fail("Silme işlemninde bir sorun oluştu!",StatusCodes.Status500InternalServerError);
            }
            var deleteResponse = _imageService.DeleteImage(product.ImageUrl!);
            if (!deleteResponse.IsSuccessful)
            {
                return ResponseDto<NoContent>.Fail("Resim silinirken bir hata oluştu", StatusCodes.Status500InternalServerError);
            }
    ...
    public async Task<ResponseDto<NoContent>> UpdateAsync(ProductUpdateDto productUpdateDto)
    {
        try
        {
            var product = await _productRepository.GetAsync(productUpdateDto.Id);
            if (product == null)
            {
                return ResponseDto<NoContent>.Fail("Ürün bulunamadı.", StatusCodes.Status404NotFound);
            }
    ...
    public async Task<ResponseDto<bool>> UpdateIsActiveAsync(int id)
    {
        try
        {
            ...
            if (product == null)
            {
                return ResponseDto<bool>.Fail("Ürün bulunumadı", StatusCodes.Status404NotFound);
            }
            ...
            if (result < 1)
            {
                return ResponseDto<bool>.Fail("Ürün Güncellenirken hata oluştu", StatusCodes.Status404NotFound);
            }
            return ResponseDto<bool>.Success(StatusCodes.Status204NoContent);
        }
        catch (Exception ex)
        {
