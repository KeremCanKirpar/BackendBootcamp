...
namespace EShop.MVC.Services.Concrete
{
    public class PaymentService : IPaymentService
    {
        public async Task<ResponseModel<bool>> CheckOutAsync(OrderCreateModel orderCreateModel)
        {
            ...
            options.ApiKey = "sandbox-ajhxtH7HUNlNN9LujKAiS4RdJWhDmo6e";
            options.SecretKey = "sandbox-YJsUvwGWtEUv2NllAzgUzJi8bxHnilf6";
            options.BaseUrl = "https://sandbox-api.iyzipay.com";

            CreatePaymentRequest request = new CreatePaymentRequest();
            request.Locale = Locale.TR.ToString();
            request.ConversationId = "123456789";
            request.Price = orderCreateModel.TotalAmount.ToString().Replace(",", ".");
